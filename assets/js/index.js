...
async function loadKontenBeranda() {
  try {
    ...
    if (eyebrow && k.s2_eyebrow) eyebrow.childNodes[eyebrow.childNodes.length - 1].textContent = k.s2_eyebrow;

    const heroSub = document.querySelector('.hero-sub');
    if (heroSub && k.s2_sub) heroSub.innerHTML = k.s2_sub.replace(/\n/g, '<br>');

    // Section 3 — About
    const sectionTag = document.querySelector('.about-sec .section-tag');
    if (sectionTag && k.s3_tag) sectionTag.childNodes[sectionTag.childNodes.length - 1].textContent = k.s3_tag;

    const sectionTitle = document.querySelector('.about-sec .section-title');
    if (sectionTitle && k.s3_title) {
      const lines = k.s3_title.split('\n');
      sectionTitle.innerHTML = lines[0] + (lines[1] ? '<br><em>' + lines[1] + '</em>' : '');
    }
...
function _clearCS() {
  Object.keys(_cST).forEach(function(id) { clearInterval(_cST[id]); });
  _cST = {};
}
...
function renderGrid() {
  ...
  if (!products.length) {
    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:60px 0"><p style="font-size:11px;color:var(--gray);letter-spacing:.2em;text-transform:uppercase">Belum ada produk</p></div>';
    return;
  }
  ...
  show.forEach(function(p) {
    ...
    if (p.price_ori > p.price) {
      priceHtml += '<span class="price-ori">Rp' + fmt(p.price_ori) + '</span><span class="price-disc">-' + disc(p) + '%</span>';
    }
    priceHtml += '</div>';
    ...
    var card = [
      ...
      '<img id="cimg_' + pid + '" src="' + firstImg + '" alt="' + p.name + '" loading="lazy" style="transition:opacity .2s" onerror="this.style.minHeight=\'200px\'"/>',
      (p.badge ? '<span class="prod-card-badge b-' + p.badge + '">' + p.badge + '</span>' : ''),
      '<button class="prod-card-wish' + (isWished ? ' wished' : '') + '" onclick="event.stopPropagation();handleToggleWish(\'' + pid + '\')">' + (isWished ? '♥' : '♡') + '</button>',
      ...
      (imgs.length > 1 ? '<div style="position:absolute;bottom:10px;left:50%;transform:translateX(-50%);display:flex;gap:4px;z-index:4;pointer-events:none">' + imgs.map(function(_, i){ return '<span class="cdot_' + pid + '" style="display:block;height:5px;border-radius:3px;transition:all .3s;background:' + (i===0?'#fff':'rgba(255,255,255,.4)') + ';width:' + (i===0?'14px':'5px') + '"></span>'; }).join('') + '</div>' : ''),
      '</div>',
